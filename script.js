...
// ===== ENGINE 3D ASSETS (per engine size) =====
...
// ===== SPEAKER & GEARBOX 3D ASSETS =====
const SPEAKER_3D_UIDS = {
  standard:    'c09ac57f5a5644adb81d1173dcf101e9',
  '6sp':       '30e6062de2a747109265e5d8d6c71f9d',
  '8sp_sub':   'c09ac57f5a5644adb81d1173dcf101e9',
  premium_bose:'3130713af65a48949a08dbaf5d68e370'
};

const GEARBOX_3D_UIDS = {
  manual: '811ce409dda746099924ac0b39d34140',
  auto:   'd7936010c8274424aa75fc7ae5d3daf3',
  dct:    '3670f48529994ef18f5411ce5eaf8046'
};

// ===== TESLA BATTERY 3D ASSETS =====
...
// ===== INTERIOR / SUSPENSION / BRAKES 3D ASSETS =====
...
// NEW: speakers + gearbox 3D DOM references
const speaker3dWrap  = document.getElementById('speaker3dWrap');
const speaker3dFrame = document.getElementById('speaker3dFrame');
const speakerImg     = document.getElementById('speakerImg');

const gearbox3dWrap  = document.getElementById('gearbox3dWrap');
const gearbox3dFrame = document.getElementById('gearbox3dFrame');
...
function useSketchfabForCurrentModel() {
  return [
    ...
    'swift','baleno','ciaz','brezza',
    'innova','fortuner',
    'audi_a4','audi_q7',
    'merc_cclass','merc_gla',
    'tesla_model3','tesla_modelx'
...
// speakers + gearbox events (update 3D + price)
if(speakerSelect) speakerSelect.addEventListener('change', ()=>{
  updateSpeakerVisual();
  updatePrice();
});
if(gearboxSelect) gearboxSelect.addEventListener('change', ()=>{
  updateGearboxVisual();
  updatePrice();
});
...
function loadComponentOptions(){
  ...
  if (engineImg)     engineImg.src     = `https://source.unsplash.com/800x480/?car-engine,${m}`;
  if (interiorImg)   interiorImg.src   = `https://source.unsplash.com/800x480/?car-interior,${m}`;
  if (suspensionImg) suspensionImg.src = `https://source.unsplash.com/800x480/?car-suspension,${m}`;
  if (brakesImg)     brakesImg.src     = `https://source.unsplash.com/800x480/?car-brake,${m}`;

  if (interiorSelect)   interiorSelect.value   = 'Standard';
  if (suspensionSelect) suspensionSelect.value = 'Standard';
  if (brakesSelect)     brakesSelect.value     = 'Standard';
...
// show 3D engine or Tesla battery
...
// Speakers 3D visual
function updateSpeakerVisual(){
  if (!speakerSelect) return;
  const val = speakerSelect.value;
  const uid = SPEAKER_3D_UIDS[val] || SPEAKER_3D_UIDS['standard'];

  if (speaker3dWrap && speaker3dFrame){
    speaker3dWrap.style.display = 'block';
    if (speakerImg) speakerImg.style.display = 'none';

    const baseUrl = `https://sketchfab.com/models/${uid}/embed`;
    speaker3dFrame.src = `${baseUrl}?autostart=1&autospin=0.3&ui_controls=1&ui_stop=0`;
  }
}

// Gearbox 3D visual
function updateGearboxVisual(){
  if (!gearboxSelect) return;
  const val = gearboxSelect.value;
  const uid = GEARBOX_3D_UIDS[val] || GEARBOX_3D_UIDS['manual'];

  if (gearbox3dWrap && gearbox3dFrame){
    gearbox3dWrap.style.display = 'block';
    if (gearboxImg) gearboxImg.style.display = 'none';

    const baseUrl = `https://sketchfab.com/models/${uid}/embed`;
    gearbox3dFrame.src = `${baseUrl}?autostart=1&autospin=0.3&ui_controls=1&ui_stop=0`;
  }
}
...
function buildCheckout(){
  ...
  checkoutBox.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:8px">
      ...
      <div><strong>Speaker System:</strong> ${speakerSelect ? speakerSelect.options[speakerSelect.selectedIndex].text : 'Standard'}</div>
      <div><strong>Gearbox:</strong> ${gearboxSelect ? gearboxSelect.options[gearboxSelect.selectedIndex].text : 'Manual'}</div>
...
// wire price + 3D triggers (original bottom listeners)
